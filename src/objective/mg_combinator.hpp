...
*/
  public:
    ...
    void setSOCPData_opt_bal(){
      ...
      A_bal.setZero(m, n);
      A_bal.block(0,0,num_mo_,num_mo_) = -Eigen::MatrixXd::Identity(num_mo_,num_mo_);
    ...
    bool GetCoefficients(const std::vector<score_t>* multi_gradients,
      ...
      Eigen::Map<const Eigen::VectorXd> costs(mo_costs, num_mo_);
      log_details.str("\n");
      double mu = CauchySchwarzMu(costs);
      bool solved;
      if (mu >= mu_thresh_) {
        log_details<<"[EPO] balancing... mu="<<mu<<std::endl;
        setSOCPData_opt_bal() ;
        GetLagrangeAnchor(costs, anchor);
...
*/
  public:
    ...
    void setSCOPData_opt_u(){
      q.setConstant(1, 1 + num_mo_);
      int m = num_mo_ + 1 + num_mo_ + (1 + num_mo_);
      int n = 1 + 1 + num_mo_ + num_mo_;
      int f = 0;
      int l = 2 * num_mo_ + 1;
...
*/
  public:
    ...
    void setSOCPData_opt_u(){
      ...
      // Set common parameters for both balance and descent mode
      q.setConstant(1, 1 + num_mo_);
      m = (num_mo_ - 1) + 1 + (num_mo_-1) + (1 + num_mo_);
      n = 1 + (num_mo_-1) + num_mo_;
...
*/
  public:
    ...
    void Init(const Metadata& metadata, data_size_t num_data) override {
      ...
      Eigen::VectorXd a = preferences_.normalized();
