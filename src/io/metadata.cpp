...
/* need to fix as it assumes all objectives including primary */
...
//  for (size_t i = 0; i < mo_data_.size(); i++) mo_data_[i] = std::vector<float>(num_data, 0.0f);
}
...
void Metadata::LoadMOQueryWeights() {
  for (data_size_t mo_idx = 0; mo_idx < num_mo(); mo_idx++) {
    // Log::Info("Loading %dth mo query weights...", mo_idx);
